    ...
    <div className="mrq-card">
      ...
      <div className="mrq-card-main">
        ...
        <div className="mrq-badge-row">
          <span className={`mrq-badge ${payPending ? "pay-pending" : "pay-completed"}`}>
            💳 {payPending ? "Payment Pending" : "Payment Done"}
          </span>
          <span className={`mrq-badge ${reqPending ? "req-pending" : "req-completed"}`}>
            {reqPending ? "⏳ Awaiting" : "✓ Processed"}
          </span>
        </div>
        ...
        <div className="mrq-info-grid" style={{ marginTop: 14 }}>
          <div className="mrq-info-item">
            ...
            <span className="mrq-info-value">
              {request.mentorId?.name || request.mentor || "—"}
            </span>
          </div>
          <div className="mrq-info-item">
            <span className="mrq-info-label">Learner</span>
            <span className="mrq-info-value">
              {request.learnerId?.name || request.learner || "—"}
            </span>
          </div>
        </div>
      ...
      <div className="mrq-card-footer">
        ...
        <button className="mrq-btn accept" onClick={() => onAccept(request._id)} title="Accept request">
          {Icon.check} Accept
        </button>
      </div>
    </div>
...
function ManageRequest() {
  ...
  const handleAccept = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        // await acceptRequest({ _id: id });
        Swal.fire({ title: "Accepted!", text: "Request has been accepted.", icon: "success", confirmButtonColor: "#10b981" });
        getAllRequest();
      }
    });
  };
  ...
  const handleReject = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        // await rejectRequest({ _id: id });
        Swal.fire({ title: "Rejected!", text: "Request has been rejected.", icon: "success", confirmButtonColor: "#6366f1" });
        getAllRequest();
      }
    });
  };
